...
def test_prompt_template_generation_first_call(benchmark):
    """
    Benchmark first call to get_file_analysis_prompt (uncached).

    Note: This measures @lru_cache overhead on first call.
    """
    # Clear cache before benchmark
    get_file_analysis_prompt.cache_clear()

    result = benchmark(get_file_analysis_prompt)
...
def test_prompt_template_generation_cached(benchmark):
    """
    Benchmark cached calls to get_file_analysis_prompt.
...
def test_prompt_template_repeated_calls(benchmark):
    ...
    def repeated_calls():
        results = []
        for _ in range(100):
            results.append(get_file_analysis_prompt())
        return results
